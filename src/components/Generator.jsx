  ...
  return (
    <SectionWrapper id={'generate'} header={"generate your workout"} title={
        ...
        <div className='bg-slate-950  border border-solid border-blue-400 rounded-lg flex flex-col'>
            <button onClick={toggleModal} className=' relative  p-3 items-center justify-center flex flex-col'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
            {Object.keys(SCHEMES).map((schemas,schemasIndex)=>{
                return(
                    <button onClick={()=>{
                        setGoal(schemas)
                    }} className={'bg-slate-950 border  duration-200 hover:border-blue-600 rounded-lg py-3 px-4 '
                    +(schemas===poison ? 'border-blue-600':'border-blue-400')}
                     key={schemasIndex}>
                        <p className='capitalize'>{schemas.replaceAll('_'," ")}</p>
                    </button>
                )
            })}
        </div>
